...
| 3 | Drift is to the right, magnitude physically plausible | **Yes, with a caveat** | Right in 15/15; reversing the rifling reverses it. Magnitude **+5.0 % to +24.8 %** (mean +14.4 %; worst where the absolute drift is only ~3 m). Plausible but not tight, and now closed as an open residual with every coefficient candidate eliminated — see [STEP1-CLOSEOUT.md](STEP1-CLOSEOUT.md) §3. |
...
1. **Drift magnitude is an open residual (~14 %), closed as unexplained.** Not
   tuned away. The C_Nα correction now in place is built from the full measured
   BRL full-scale set and was adopted on the centre-of-pressure evidence even
   though it makes drift *worse* (+10.2 % → +14.4 %) — that it does so is the
   finding. Every coefficient candidate has been tested against measurement and
   eliminated: twist, C_Nα, C_Mα, uniform rescale, the missing pitch-damping
   force (0.70 % of the normal force), Coriolis, sign, timestep. The remaining
   lead is a model-class question that belongs to step 2. Full account in
